...
import org.agmip.core.types.TranslatorIO;
...
public class QuadUIWindow extends Window implements Bindable {
	...
	private Writer exWriter = new StringWriter();
	private PrintWriter exOutput = new PrintWriter(exWriter);
	...
	private ArrayList<String> validateInputs() {
		...
		for( Checkbox cbox : checkboxGroup) {
			if (cbox.isSelected()) anyModelChecked = true;
		}
		if (! anyModelChecked) {
